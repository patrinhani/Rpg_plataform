...
O fog começa **ligado e totalmente fechado** em todas as cenas. Quando a campanha possui setores oficiais, eles já são carregados no primeiro acesso; o Mestre ainda pode ajustar polígonos, restaurar apenas a cena ativa ou reaplicar os setores oficiais em todos os mapas. As restaurações substituem desenhos manuais somente depois de uma confirmação explícita. Durante a sessão, o modo de controle permite alternar uma região com um clique. Se o mapa ativo da cena mudar entre versões da campanha, as regiões anteriores são substituídas pelo preset atual, sempre fechado, para evitar revelações acidentais.
